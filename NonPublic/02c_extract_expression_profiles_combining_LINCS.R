...
combined_matrices = cbind(subset_matrix_lincs1_matching_genes, subset_matrix_lincs2_matching_genes)

combined_matrices[1:5, 1:5]

#combine the annotation files

drugdata_total = rbind(#DrugData_cmap, 
                       DrugData_lincs1, 
                       DrugData_lincs2)
head(drugdata_total)
...
fwrite(x = exprs_combined_krubor, 
       file = "results/krubor_combined_lincs_1_and_2_ai.txt", 
       quote = FALSE, 
       sep = "\t", 
       row.names = TRUE, 
       col.names = TRUE)
